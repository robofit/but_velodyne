...
Mat Image::computeIDTEdgeImage(Mat &edge_img) {
	...
	for (int rows_cols = 0; rows_cols < 2; rows_cols++) {
		...
		for (int row = 0; row < edges.rows; row++) {
			for (int x = 0; x < width; x++) {
				...
				new_weights.at<float>(row, x) = sub_weights.at<float>(
						max_loc.x);
			}
		}
	...
	addWeighted(default_edges, alpha, idt_edge_img, (1.0 - alpha), 0.0,
			idt_edge_img);
...
Mat Image::computeIDTEdgeImage() {
	...
	Mat idt_edge_img = this->computeIDTEdgeImage(edge_img).mul(
			cv::Scalar::all(255.0));
