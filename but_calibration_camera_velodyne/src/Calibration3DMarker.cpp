...
Calibration3DMarker::Calibration3DMarker(cv::Mat _frame_gray, cv::Mat _P,
		pcl::PointCloud<Velodyne::Point> _pc, float _circ_distance,
		float _radius) :
		frame_gray(_frame_gray), P(_P), pc(_pc), circ_distance(
				_circ_distance), radius(_radius) {
	...
	pcl::PointCloud<pcl::PointXYZ>::Ptr xyz_cloud_ptr(
			thresholded_scan.toPointsXYZ());

	pcl::SampleConsensusModelPlane<pcl::PointXYZ>::Ptr model_p(
			new pcl::SampleConsensusModelPlane<pcl::PointXYZ>(
					xyz_cloud_ptr));
	...
	copyPointCloud<pcl::PointXYZ>(*xyz_cloud_ptr, inliers_indicies, plane);

	// ---------------- REMOVE LINES ----------------

	for (int i = 0; i < 2; i++) {
		pcl::PointCloud<pcl::PointXYZ>::Ptr plane_ptr(
				new pcl::PointCloud<pcl::PointXYZ>(plane));
...
bool Calibration3DMarker::detectCirclesInImage(vector<Point2f> &centers,
	...
	return img_edge.detect4Circles(Calibration3DMarker::CANNY_THRESH,
			Calibration3DMarker::CENTER_THRESH_DISTANCE, centers, radiuses);
}
...
bool Calibration3DMarker::detectCirclesInPointCloud(vector<Point3f> &centers,
	...
	*detection_cloud += this->plane;
	...
	int round = 1;
	vector<pcl::PointXYZ> spheres_centers;
	bool detected = false;
	for (int iterations = 0; iterations < 64; iterations++) {
		...
		if (spheres_centers.size() == 4) {
			...
			if (verify4spheres(spheres_centers, this->circ_distance,
					tolerance)) {
				spheres_centers = refine4centers(spheres_centers,
						detection_cloud);
				detected = true;
				break;
			}
		}
		vector<pcl::PointXYZ> possible_centers = generate_possible_centers(
				spheres_centers, this->circ_distance);
		generate_possible_points(this->plane, detection_cloud,
				possible_centers, this->circ_distance, 0.01);
	}
	...
	for (size_t i = 0; i < spheres_centers.size(); i++) {
		centers.push_back(
				Point3f(spheres_centers[i].x, spheres_centers[i].y,
						spheres_centers[i].z));
	}
	return true;
}
...
vector<pcl::PointXYZ> Calibration3DMarker::detect4spheres(pcl::PointCloud<pcl::PointXYZ>::Ptr plane,
		vector<float> &radiuses) {

	radiuses.clear();
	vector<pcl::PointXYZ> centers;
	std::vector<int> inliers_indicies;
	pcl::PointCloud<pcl::PointXYZ> *four_spheres = new pcl::PointCloud<
			pcl::PointXYZ>();
	float tolerance = 0.02;
	for (int i = 0; i < 4; i++) {
		pcl::SampleConsensusModelSphere<pcl::PointXYZ>::Ptr model_s(
				new pcl::SampleConsensusModelSphere<pcl::PointXYZ>(plane));
		...
		pcl::PointCloud<pcl::PointXYZ>::Ptr inliers(
				new pcl::PointCloud<pcl::PointXYZ>);
		remove_inliers<pcl::PointXYZ>(*plane, inliers_indicies, *outliers);
		pcl::copyPointCloud<pcl::PointXYZ>(*plane, inliers_indicies, *inliers);
		plane = outliers;
		//view(plane);
	...
	pcl::PointCloud<pcl::PointXYZ>::Ptr four_spheres_ptr(four_spheres);
	return centers;
}
 ...
 * 2 3
 */
bool orderX(pcl::PointXYZ p1, pcl::PointXYZ p2) {
	return p1.x < p2.x;
}
bool orderY(pcl::PointXYZ p1, pcl::PointXYZ p2) {
	return p1.y < p2.y;
}

void Calibration3DMarker::order4spheres(vector<pcl::PointXYZ> &spheres_centers) {
	assert(spheres_centers.size() == 4);
...
float euclid_dist(const pcl::PointXYZ p1, const pcl::PointXYZ p2) {
	return sqrt(pow(p1.x - p2.x, 2) + pow(p1.y - p2.y, 2) + pow(p1.z - p2.z, 2));
}

bool Calibration3DMarker::verify4spheres(const vector<pcl::PointXYZ> &spheres_centers,
	...
	bool res = true;
	for (vector<pair<int, int> >::iterator neighbors =
			neighbour_indexes.begin(); neighbors < neighbour_indexes.end();
			neighbors++) {
		float error = abs(
				euclid_dist(spheres_centers[neighbors->first],
						spheres_centers[neighbors->second])
						- straight_distance);
 ...
 * x x x
 */
vector<pcl::PointXYZ> Calibration3DMarker::generate_possible_centers(
		const vector<pcl::PointXYZ> &spheres_centers, float straight_distance) {
	vector<pcl::PointXYZ> possible_centers;

	for (vector<pcl::PointXYZ>::const_iterator c = spheres_centers.begin();
			c < spheres_centers.end(); c++) {
...
void Calibration3DMarker::generate_possible_points(pcl::PointCloud<pcl::PointXYZ> &plane,
		pcl::PointCloud<pcl::PointXYZ>::Ptr detection_cloud,
		const vector<pcl::PointXYZ> &possible_centers, float radius,
		float tolerance) {
	...
	for (PointCloud<PointXYZ>::iterator pt = plane.begin(); pt < plane.end();
			pt++) {
		int votes = 0;
		for (vector<pcl::PointXYZ>::const_iterator center =
				possible_centers.begin(); center < possible_centers.end();
				center++) {
...
vector<PointXYZ> Calibration3DMarker::refine4centers(vector<PointXYZ> centers,
	...
	for (vector<PointXYZ>::iterator c = centers.begin(); c < centers.end();
			c++) {
		c->z = z_coord;
	}
