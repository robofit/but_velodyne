...
Velodyne::Velodyne Velodyne::Velodyne::transform(float x, float y, float z,
...
Mat Velodyne::Velodyne::project(Mat projection_matrix, Rect frame,
		PointCloud<Point> *visible_points) {
	Mat plane = cv::Mat::zeros(frame.size(), CV_32FC1);

	for (pcl::PointCloud<Point>::iterator pt = point_cloud.points.begin();
	...
	for (vector<vector<Point*> >::iterator ring = rings.begin();
		...
		for (vector<Point*>::iterator pt = ring->begin() + 1;
			...
			case Processing::DISTORTIONS:
				(*pt)->intensity =
						MAX( MAX( prev->range-(*pt)->range, succ->range-(*pt)->range), 0)
								* 10;
				break;
			case Processing::INTENSITY_EDGES:
				new_intensity =
						MAX( MAX( last_intensity-(*pt)->intensity, succ->intensity-(*pt)->intensity), 0)
								* 10;
...
vector<Velodyne::Velodyne> Velodyne::Velodyne::depthSegmentation(
		int segment_counts) {
...
PointCloud<PointXYZRGB> Velodyne::Velodyne::colour(cv::Mat frame_rgb,
		cv::Mat P) {
