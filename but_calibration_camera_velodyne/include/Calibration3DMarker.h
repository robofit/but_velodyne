...
protected:
	...
	void remove_inliers(
			...
			std::vector<int> inliers_indices, pcl::PointCloud<PointT> &cloud_out) {
	...
	bool verify4spheres(const std::vector<pcl::PointXYZ> &spheres_centers,
	...
	void generate_possible_points(pcl::PointCloud<pcl::PointXYZ> &plane,
			...
			const std::vector<pcl::PointXYZ> &possible_centers, float radius,
			float tolerance);
