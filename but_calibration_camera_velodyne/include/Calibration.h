...
public:
	Calibration6DoF(float x, float y, float z, float x_r, float y_r, float z_r,
			float val) {
	...
	void set(float x, float y, float z, float x_r, float y_r, float z_r,
			float val) {
		value = val;
	...
	void print(void) {
		std::cout << "score: " << value << ";\t 6DoF: [" << DoF[0] << " " << DoF[1] << " " << DoF[2]
...
class Calibration {
public:
	static CalibrationInputs loadArgumets(int argc, char *argv[], bool DoF =
			false) {
	...
	static Calibration6DoF findTranslation(std::vector<cv::Point2f> image,
			std::vector<cv::Point3f> velodyne, cv::Mat projection,
			float radius2D, float radius3D) {
		...
		// t_z:
		translation[INDEX::Z] = radius3D * focal_len / radius2D
				- velodyne.front().z;
		...
		for (size_t i = 0; i < image.size(); i++) {
			...
			translation[INDEX::X] += (image[i].x - principal_x)
					* (velodyne[i].z + translation[INDEX::Z]) / focal_len
					- velodyne[i].x;
			// t_y:
		...
		return Calibration6DoF(translation[INDEX::X], translation[INDEX::Y],
				translation[INDEX::Z], 0, 0, 0, 0);
	}

	static void calibrationRefinement(Image::Image img, Velodyne::Velodyne scan,
			cv::Mat P, float x_rough, float y_rough, float z_rough,
		...
		Velodyne::Velodyne transformed = scan.transform(x_rough, y_rough,
				z_rough, 0, 0, 0);
		float rough_val = Similarity::edgeSimilarity(img, transformed, P);

		best_calibration.set(x_rough, y_rough, z_rough, 0, 0, 0,
				rough_val);
		//cout << "rough:\t";
					...
					for (size_t x_ri = 0; x_ri < steps; x_ri++) {
						...
						for (size_t y_ri = 0; y_ri < steps; y_ri++) {
							...
							for (size_t z_ri = 0; z_ri < steps; z_ri++) {
								...
								Velodyne::Velodyne transformed = scan.transform(
										x, y, z, x_r, y_r, z_r);
								float value = Similarity::edgeSimilarity(img,
										transformed, P);
								Calibration6DoF calibration(x, y, z, x_r, y_r,
										z_r, value);
								if (value > best_calibration.value) {
									best_calibration.set(x, y, z, x_r, y_r,
											z_r, value);
								}
