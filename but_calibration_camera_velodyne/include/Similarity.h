...
public:
	...
	float static projectionError(cv::Mat &segmentation,
			std::vector<Velodyne::Velodyne> &segments, cv::Mat P, bool verbose =
					false) {
		...
		for (int i = 0; i < 2; i++) {
			...
			for (pcl::PointCloud<Velodyne::Point>::iterator pt =
					segments[i].begin(); pt < segments[i].end(); pt++) {
