...
/**
* Class for computing LBP histogram
*
*
...
LBP::LBP(int radius=1,int neighbors=8,int _type=ROTARY_INVARIANT_OLBP):_radius(radius),_neighbors(neighbors),type(_type),max_rotary_invariantion_bin(0) 
{
	...
	else if (type==ROTARY_INVARIANT_LBP || type==ROTARY_INVARIANT_OLBP  )
	{
		initRotaryInvariantLookup();
	}
	else if (type==ELBP || type==OLBP)
	{
	...
	for(int q=0; q<src.channels(); q++)
	{
		...
		for(i=1;i<src.rows-1;i++) {
			...
			for(j=1;j<src.cols-1;j++) {
				...
				if ( type==UNIFORM_OLBP ) // replace content of dst with its resp. lookup value
						code= lookup[code];

				if ( type==ROTARY_INVARIANT_OLBP ) // replace content of dst with its resp. lookup value
						code= rilookup[code];
...
{
	if(type==ROTARY_INVARIANT_OLBP || type==OLBP || type==UNIFORM_OLBP)
