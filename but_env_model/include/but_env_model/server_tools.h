...
template <typename tpType> tpType abs(tpType x)
{
  return x < 0.0 ? -x : x;
}

//! Is absolute value of the given number smaller than SMALL_DOUBLE?
template <typename tpType> bool isSmall(tpType x)
{
  return abs(x) < SMALL_DOUBLE;
}

//! Is absolute value of the given number greater than SMALL_DOUBLE?
template <typename tpType> bool isGreat(tpType x)
{
  return abs(x) > SMALL_DOUBLE;
}
